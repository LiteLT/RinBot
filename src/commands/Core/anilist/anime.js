...
module.exports = class extends Subcommand {
    ...
    async run(message, args) {
        ...
        let query = `
        query AnimeData($search: String!, $perPage: Int!, $plain: Boolean!, $reviewLimit: Int!) {
            Page(perPage: $perPage) {
                media(type: ANIME, search: $search${adult}) {
                    ...media
                    idMal @skip(if: $plain)
                    format
                    ...
                    streamingEpisodes @skip(if: $plain) { url title }
                    nextAiringEpisode { episode timeUntilAiring }
                    rankings { rank type year season allTime }
                    studios(isMain: true) @skip(if: $plain) { nodes { name siteUrl } }
        ...
        let animeData = (await this._request(query, variables)).data.Page.media;
        let media = null;

        if (animeData.length) {
            if (flags.all) {
                media = await this.prompt(message, flags, animeData, variables.search);
            } else {
                [media] = animeData;
            }
        } else {
        ...
        if (media) {
            ...
            if (media.idMal) {
                media.externalLinks.push({
                    site: "MyAnimeList",
                    url: Endpoints.MYANIMELIST_ANIME(media.idMal)
                });
            }
    ...
    async result(message, flags, media, sendType) {
        ...
        let rankedScore = media.rankings
            .find((rank) => rank.type === "RATED" && (rank.allTime || rank.year || rank.season));

        let rankedPopularity = media.rankings
            .find((rank) => rank.type === "POPULAR" && (rank.allTime || rank.year || rank.season));
        ...
        if (sendType === "embed") {
                    ...
                    fields: [
                        {
                            ...
                            value: [
                                media.nextAiringEpisode
                                    ? `Airing: **Episode ${media.nextAiringEpisode
                                        .episode}** ${emdash} ${ms(media.nextAiringEpisode
                                        .timeUntilAiring * 1000, { long: true })} from now.`
                                    : null,
                                `Format: **${mediaFormat(media.format) || "Unknown"}**`,
                                `Episodes: **${media.episodes || "N/A"}** ${media.duration
                                ...
                                `Studio${media.studios.nodes.length > 1 ? "s" : ""}: ${media.studios
                                    .nodes.map((studio) => `[${studio.name}](${studio.siteUrl})`)
                                    .join(", ") || "**N/A**"}`,
                                `Alternative Titles:\n` + [
            ...
            let infoStats = "\n\n**Information**\n" + [
                media.nextAiringEpisode
                    ? `Airing: **Episode ${media.nextAiringEpisode.episode}** ${emdash} ${ms(media
                        .nextAiringEpisode.timeUntilAiring * 1000, { long: true })} from now.`
                    : null,
                `Format: **${mediaFormat(media.format) || "Unknown"}**`,
                `Episodes: **${media.episodes || "N/A"}** ${media.duration
                    ? `(**${media.duration}** minutes each)`
        ...
        if (msg && [undefined, true].includes(message.channel.permissionsOf?.(this.client.user.id)
            ...
            let pageLayouts = [
                ...
                media.streamingEpisodes.length ? {
                    ...embedTemplate,
                    url: `${embedTemplate.url}/watch`,
                    title: `${embedTemplate.title} > Watch`,
                    description: Util.arrayJoinLimit(media.streamingEpisodes.map((episode) => `${bulletChar} [${episode
                        .title.replace(/[\[\]]/g, (str) => `\\${str}`)}](${episode.url})`), "\n", Constants.Discord
        ...
        let content = sendType === "embed"
            ? {
                embed: {
                    ...
                    title: prefix + `"${search.substring(0, Constants.Discord.MAX_EMBED_TITLE_LENGTH - prefix
                        .length - 2)}"`,
