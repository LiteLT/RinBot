        ...
        let lessons = [
            {
                message: `You're about to take the interactive lesson on how to use **${this.client.user
                    .username}**. The lesson should take about 5-10 minutes to complete. If you feel like changing ` +
                    "the lesson up a bit, here are some built-in features for this.\n\n" + [
            ...
            },
            {
                ...
                `Use the \`help\` command to get info on the \`userinfo\` command. Usage: \`${message.prefix}help ` +
                "<command>`.",
            ...
            },
            {
                ...
                "outcome of the command. For example, most utility commands come with a `--noembed` flag to send the " +
                "message as plain text instead of an embed. Flags must start with `--` and be 2+ letters long. Flags " +
                "are not included in the content of your message.\n\nMission: Use the `userinfo` command with the " +
                "`noembed` flag by appending `--flagname` to the end of your message.",
            ...
            },
            {
                ...
                "executing the command easier than typing the whole name. For example, the `userinfo` command has an " +
                "alias of `ui`. If you replaced the command name for its alias, it would still work.\n\nMission: Use " +
                "the alias `si` to execute the `serverinfo` command. If you're in a DM channel, don't worry if it " +
                "doesn't show the guild info. It's supposed to do that.",
